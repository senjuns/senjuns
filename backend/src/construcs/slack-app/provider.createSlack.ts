...
export async function handler(event: OnEventRequest): Promise<OnEventResponse> {
  ...
  if (!accessToken || isExpired(secret.exp ?? 0)) {
    ...
    const rotate = await slackClient
      .get('tooling.tokens.rotate', {
    ...
    const putSecretValue = await secretsmanager
      .putSecretValue({
        SecretId: event.ResourceProperties.configurationTokenSecretArn,
      ...
      .promise();
    console.log(
      `Successfully saved access token in secret ${putSecretValue.ARN}`,
    );
  }
  ...
  if (!response.ok) {
    const errors = response.errors
      ? response.errors
        .map((err) => `${err.message} at ${err.pointer}`)
        .join('\n')
      : '';
    throw new Error(
      `Failed to ${operation} manifest: ${response.error}.${
        errors ? `\n${errors}}` : ''
      }`,
    );
  }
  ...
  console.log(
    `Successfully ${operation}d Slack app ${
      event.PhysicalResourceId ?? response.app_id
    }`,
  );
  ...
  if (event.RequestType === 'Create' && response.credentials) {
    ...
    const putSecretValue = await secretsmanager
      ...
      .promise();
    console.log(
      `Successfully saved app credentials in secret ${putSecretValue.ARN}`,
    );
  }
