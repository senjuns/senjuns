        ...
        interactivity: this.props.interactivity
          ? {
            ...
            message_menu_options_url:
                this.props.interactivity?.messageMenuOptionsUrl,
          }
          : undefined,
      ...
      features: {
        app_home: {
          ...
          messages_tab_read_only_enabled:
            this.props.appHome?.messagesTabReadOnly,
        },
...
function prefixWith<T extends string | undefined>(
  prefix: string,
  string: T,
): T {
  if (!string) {
...
function validateLength(description: string, max: number, xs?: string): void {
  if (xs && !Token.isUnresolved(xs) && xs.length > max) {
    throw new Error(
      `Maximum length for ${description} is ${max}, got ${xs.length}: ${xs}`,
    );
  }
}
...
function validateItems<T>(description: string, max: number, xs?: T[]): void {
  if (xs && !Token.isUnresolved(xs) && xs.length > max) {
    throw new Error(
      `Maximum number of items for ${description} is ${max}, got ${xs.length}`,
    );
  }
}
