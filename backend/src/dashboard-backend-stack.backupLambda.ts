...
export async function handler(
  ...
  if (
    (event as lambda.CloudFormationCustomResourceEvent).RequestType === 'Create'
  ) {
    return;
  }
  ...
  users.Users?.map((user) => {
    ...
    const newCognitoCsv: CognitoCsv = {
      name: user.Username || '',
      given_name:
        user.Attributes?.filter((attr) => attr.Name === 'given_name')[0]
          ?.Value || 'given_name',
      family_name:
        user.Attributes?.filter((attr) => attr.Name === 'family_name')[0]
          ?.Value || 'family_name',
      middle_name: '',
      nickname: '',
      preferred_username: '',
      ...
      website: '',
      email:
        user.Attributes?.filter((attr) => attr.Name === 'email')[0]?.Value ||
        '',
      email_verified:
        user.Attributes?.filter((attr) => attr.Name === 'email_verified')[0]
          ?.Value || 'FALSE',
      gender: '',
      birthdate: '',
      zoneinfo: '',
      locale: '',
      phone_number:
        user.Attributes?.filter((attr) => attr.Name === 'phone_number')[0]
          ?.Value || '123',
      phone_number_verified:
    ...
    const userCsvArray = [
      newCognitoCsv.name,
      newCognitoCsv.given_name,
      newCognitoCsv.family_name,
      newCognitoCsv.middle_name,
      newCognitoCsv.nickname,
      newCognitoCsv.preferred_username,
      newCognitoCsv.profile,
      newCognitoCsv.picture,
      newCognitoCsv.website,
      newCognitoCsv.email,
      newCognitoCsv.email_verified,
      newCognitoCsv.gender,
      newCognitoCsv.birthdate,
      newCognitoCsv.zoneinfo,
      newCognitoCsv.locale,
      newCognitoCsv.phone_number,
      newCognitoCsv.phone_number_verified,
      newCognitoCsv.address,
      newCognitoCsv.updated_at,
      newCognitoCsv['cognito:mfa_enabled'],
      newCognitoCsv['cognito:username'],
      newCognitoCsv['custom:organization_id'],
    ];
